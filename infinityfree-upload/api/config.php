...
// Load configuration without requiring an external dotenv dependency.
...
$db_host = $env_values['DB_HOST'] ?? (getenv('DB_HOST') ?: '');
$db_user = $env_values['DB_USER'] ?? (getenv('DB_USER') ?: '');
$db_password = $env_values['DB_PASSWORD'] ?? (getenv('DB_PASSWORD') ?: '');
$db_name = $env_values['DB_NAME'] ?? (getenv('DB_NAME') ?: '');
$db_port = $env_values['DB_PORT'] ?? (getenv('DB_PORT') ?: 3306);

$missing_db_settings = [];
foreach (['DB_HOST' => $db_host, 'DB_USER' => $db_user, 'DB_NAME' => $db_name] as $key => $value) {
    if ($value === '') {
        $missing_db_settings[] = $key;
    }
}
if ($missing_db_settings) {
    http_response_code(500);
    error_log('Database configuration is incomplete. Check the .env file.');
    die(json_encode(['error' => 'Database configuration is incomplete', 'missing' => $missing_db_settings]));
}
